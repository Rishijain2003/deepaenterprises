...
export function MiscellaneousElectronicsPage() {
  const [sortBy, setSortBy] = useState('name');
  const [filterBy, setFilterBy] = useState('all');
  const [viewMode, setViewMode] = useState<'grid' | 'list'>('grid');
  const [showFilters, setShowFilters] = useState(false);

  const miscElectronicsProducts = products.filter(product => product.category === 'Miscellaneous Electronics');
  
  const subcategories = [...new Set(miscElectronicsProducts.map(product => product.subcategory))];

  const filteredAndSortedProducts = useMemo(() => {
    let filtered = miscElectronicsProducts;
  ...
  }, [miscElectronicsProducts, sortBy, filterBy]);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <section className="bg-gradient-to-r from-[#003366] to-[#004080] text-white py-16">
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto text-center">
            <h1 className="text-4xl md:text-5xl font-bold mb-6">Miscellaneous Electronics</h1>
            <p className="text-xl text-gray-200">
              Essential electronic accessories and gadgets to enhance your digital lifestyle
            </p>
          </div>
        </div>
      </section>
      ...
      <div className="container mx-auto px-4 py-4">
        <nav className="text-sm text-gray-600">
          ...
          <span className="text-gray-900">Miscellaneous Electronics</span>
        </nav>
      </div>
            ...
            {subcategories.map((subcategory) => (
              ...
              >
                ...
                <p className="text-sm text-gray-600">
                  {miscElectronicsProducts.filter(p => p.subcategory === subcategory).length} products
                </p>
              </Link>
            ))}
          </div>
